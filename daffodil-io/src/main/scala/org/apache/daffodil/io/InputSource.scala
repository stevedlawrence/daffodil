...
class BucketingInputSource(
  ...
  */
  ...
  def getByteBuffer(): (ByteBuffer, Boolean) = {
    ...
    // getByteBuffer and changed these values.
...
class ByteBufferInputSource(byteBuffer: ByteBuffer) extends InputSource {
  ...
  // Creates a read-only ByteBuffer used only for calls to getByteBuffer. This avoid allocating
  ...
  // that shares the content of the underlying byte buffer. The
  ...
  def getByteBuffer(): (ByteBuffer, Boolean) = {
    ...
    // match the underlying bytebuffer. We also return true to signify there is no more data
    // after this buffer.
    bbForGetByteBuffer.position(bb.position).limit(bb.limit)
    (bb, true)
  }
