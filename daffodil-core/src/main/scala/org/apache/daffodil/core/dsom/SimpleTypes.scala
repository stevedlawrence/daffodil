  ...
  lazy val optTypeCalculator: Option[TypeCalculator] = LV('optTypeCalculator) {
    optRepType.flatMap(repType => {
      ...
      val fromRestriction: Option[TypeCalculator] = optRestriction.flatMap { restriction =>
        if (restriction.enumerations.isEmpty) {
          ...
          if (terms.isEmpty) {
            if (srcType != dstType) {
              SDE(
                """An enumeration without any dfdlx:repValues cannot have a different primitve type (%s) than that of its dfdlx:repType="%s" (%s)""",
                dstType.globalQName,
                repType.namedQName,
                srcType.globalQName,
              )
            ...
            if (terms.size != restriction.enumerations.size) {
              SDE(
                "If one enumeration value defines a dfdlx:repValue, then all must define a dfdlx:repValue",
              )
            }
      ...
      val ans = (fromRestriction, fromUnion) match {
        ...
        case (None, None) => {
          SDE(
            "dfdlx:repType (%s) requires an enumeration or union with defined repValues",
            repType.namedQName,
          )
        }
        case (Some(_), Some(_)) =>
