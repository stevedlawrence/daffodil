...
abstract class TypeCalculator(val srcType: NodeInfo.Kind, val dstType: NodeInfo.Kind)
  ...
  /**
   * The inputTypeCalc function provides only the input type calc logic, returning either the
   * value or an error string. This allows it to be agnostic about how to handle the resulting
   * value or error, letting a caller figure out how to best handle it. Most callers should call
   * the inputTypeCalcParse function instead of this, since it contains logic to handle the
   * result and convert to a ParseError
   */
  def inputTypeCalc(
  ...
  /**
   * The outputTypeCalc function provides only the output type calc logic, returning either the
   * value or an error string. This allows it to be agnostic about how to handle the resulting
   * value or error, letting a caller figure out how to best handle it. Most callers should call
   * the outputTypeCalcUnparse function instead of this, since it contains logic to handle the
   * result and convert to an UnparseError
   */
  def outputTypeCalc(
    x: DataValuePrimitive,
    xType: NodeInfo.Kind,
