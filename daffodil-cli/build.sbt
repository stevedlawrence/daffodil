...
// define _source_payload and _binary_payload to use gzip compression. Addition,
// the bulk of the RPM is jars which are already compressed and won't really
// compress any further, so we set the compression level to zero.
//
...
// things like CFLAGS, CXXFLAGS, etc. and the way qw use rpmbuild does not use
// this flags, since it just packages files from built by SBT.
//
...
// descriptions of each installed file. But these descriptions are created by
// libmagic, and different systems with different versions of libmagic could
// create different descriptions. RPM also includes a PLATFORM tag that usually
// includes the distribution (e.g. redhat vs debian), again something the spec
// fiel cannot change. And RPM also includes the version of RPM used to build
// the RPM file. All that to say that although we can minimze differences by
// changing modifying some macros, the same or very similar environment is still
// needed for byte exact reproducible RPM builds. However, this is usually
// enough for the rpmdiff tool to consider different builds the same.
